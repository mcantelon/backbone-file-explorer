      ...
      if (this.closeDirsByDefault) {
        ...
        $(destEl).hover(function() {
          if (!rendered) {
            ...
            $('.backbone-file-explorer-entry').removeClass('backbone-file-explorer-entry-odd');
            $('.backbone-file-explorer-entry:visible:odd').addClass('backbone-file-explorer-entry-odd');

            // re-bind drag/drop
            if (self.explorer.moveHandler) {
              //self.explorer.initDragAndDrag();
console.log($(destEl).children('div:first'))
              $(destEl)
                .bind('drag', {'self': self.explorer}, self.explorer.dragHandler)
                .bind('drop', {'self': self.explorer}, self.explorer.dropHandler);
            }
