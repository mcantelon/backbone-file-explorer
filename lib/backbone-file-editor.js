...
var Directory = Backbone.Model.extend({
  ...
  id: function() {
    return (this.get('parent') + '/' + this.get('name')).replace(/\//g, '_');
  },
...
var EntryView = Backbone.View.extend({
  ...
  render: function() {
    ...
    if (this.model.children == undefined) {
      ...
      $(this.el).children('.directory_icon_button').removeClass('directory_icon_button');
    } else {
      // set CSS ID for directory entries (used to capture whether directory
      // is open/closed by user between data refreshes)
...
var DirectoryView = Backbone.View.extend({
  ...
  appendChildElements: function(destEl, entry, level) {
    var level = level || 1
      , container = $(this.levelTemplate());

    $(destEl).append(container);

    // if entry is a directory, render children to container
  ...
  restoreToggledFolders: function() {
    for (var index in this.toggled) {
      ...
      if (toggleEl.next().is(':visible')) {
        togglEl.addClass('directory_open');
      } else {
        toggleEl.removeClass('directory_open');
      }
    }
  },
